...
object TenseHandler {
    ...
    fun createWrongAnswer(
        ...
        when (tense) {
            Tense.PRESENT_SIMPLE -> {
                ...
                if (subject.isBlank()) {
                    if (person == Person.IT) return toPresentSimpleSFrom(
                        verb
                    )
            ...
            Tense.PRESENT_PERFECT -> {
                ...
                return "$begin ${thirdForm(
                    verb
                )}"
            }
            Tense.PRESENT_PERFECT_CONTINUOUS -> {
                val begin = "${have()} $subject".trim()
                return "$begin been ${addIng(
                    verb
                )}"
            }
            Tense.PAST_SIMPLE -> {
            ...
            Tense.FUTURE_CONTINUOUS -> {
                val begin = "will $subject".trim()
                return "$begin be ${addIng(
                    verb
                )}"
            }
            Tense.FUTURE_PERFECT -> {
                val begin = "will $subject".trim()
                return "$begin have ${thirdForm(
                    verb
                )}"
            }
            Tense.FUTURE_PERFECT_CONTINUOUS -> {
                val begin = "will $subject".trim()
                return "$begin have been ${addIng(
                    verb
                )}"
            }
        }
    ...
    private fun thirdForm(verb: String) = IrregularVerb.byFirst(verb)?.third?.first() ?: addEd(
        verb
    )
    private fun secondForm(verb: String, person: Person) =
        IrregularVerb.secondForm(verb, person) ?: addEd(
            verb
        )
